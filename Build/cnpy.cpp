...
// Bill Karsh: Deleted npz handling. Lighter, no zip dependence.
...
template<typename T>
static void transpose( T* A, size_t w, size_t h )
{
    size_t          n = w * h;
    std::vector<T>  t( n );
...
cnpy::NpyArray load_the_npy_file(FILE* fp) {
    ...
    cnpy::NpyArray arr(shape, word_size, type, fortran_order);
    ...
    if( fortran_order && shape.size() == 2 ) {
        ...
        switch( word_size ) {
            case 1:
                transpose( arr.data<char>(), shape[0], shape[1] );
            break;
            case 2:
                transpose( arr.data<short>(), shape[0], shape[1] );
            break;
            case 4:
                transpose( arr.data<float>(), shape[0], shape[1] );
            break;
            case 8:
                transpose( arr.data<double>(), shape[0], shape[1] );
            break;
        }
    }
